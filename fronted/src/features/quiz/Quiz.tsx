...
const Quiz: React.FC = () => {
    ...
    useEffect(() => {
        if (!data) return;
        if (askedQuestions.some(q => normalize(q) === normalize(data.question))) return;
    ...
    const renderStars = (score: number) => {
        ...
        const emptyStars = MAX_QUESTIONS - fullStars - (halfStar ? 1 : 0)-5;

        for (let i = 0; i < fullStars; i++) {
            stars.push(<span key={`full-${i}`} style={{ color: 'gold', fontSize: 24, marginRight: 4 }}>★</span>);
        }

        if (halfStar) {
            stars.push(
                <span key="half" style={{ color: 'gold', fontSize: 24, marginRight: 4, position: 'relative', display: 'inline-block', width: 24 }}>
                    <span style={{ position: 'absolute', overflow: 'hidden', width: '50%', display: 'inline-block', color: 'gold' }}>★</span>
        ...
        for (let i = 0; i < emptyStars; i++) {
            stars.push(<span key={`empty-${i}`} style={{ color: '#ccc', fontSize: 24, marginRight: 4 }}>★</span>);
        }
    ...
    return (
        <div style={{ maxWidth: 600, margin: '40px auto', padding: 20, direction: 'rtl', fontFamily: 'Segoe UI' }}>
            {!quizFinished && (
                <>
                    <input
                        type="text"
            ...
            {questionText && <div style={{ marginBottom: 20, fontSize: 20, fontWeight: 'bold' }}>{questionText}</div>}
            ...
            {result && (
                <div style={{ marginBottom: 20, fontSize: 18, color: result.includes('נכון') ? 'green' : 'red' }}>
                    {result}
                </div>
            )}
